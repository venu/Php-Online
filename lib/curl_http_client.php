...
class Curl_HTTP_Client
{
	 ...
	 * @access public
	 */
	function send_post_data($url, $postdata, $ip=null, $timeout=150)
	{
		...
		//$this->request_headers['Content-Type'] = 'application/x-www-form-urlencoded; charset=UTF-8';
		...
		else 
		{
			...
			//$post_string = str_replace(array("%3D","%26"),array("=","&"),$postdata);
		}
		if($this->debug)
		{
	 ...
	 * @access public
	 */
	function fetch_url($url, $ip=null, $timeout=90)
	{
		if($this->isXMLHttpRequest)
			$this->request_headers['X-Requested-With'] = 'XMLHttpRequest';
		if(is_array($this->request_headers) && count($this->request_headers) > 0) {
	...
	{		
		//echo "set_cookie called".$cookie."<br>";
